...
def bulma_inline(element):
    wrapper_classes = element.field.widget.attrs.pop("wrapper_classes", "")
    markup_classes = {
        "label": "sr-only",
        "value": "",
        "single_value": "",
...
def add_input_classes(field):
    if (
        ...
        if "control" not in field_classes:
...
def render(element, markup_classes):
    if isinstance(element, BoundField):
        ...
        wrapper_classes = element.field.widget.attrs.pop(
            "wrapper_classes", "column is-12 py-0"
        )
        markup_classes["wrapper"] = wrapper_classes
