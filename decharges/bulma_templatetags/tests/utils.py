...
def element_has_all_attributes(element, attributes):
    for attribute_name, attribute_value in attributes.items():
        ...
        assert (
            element.get(attribute_name) == attribute_value
        ), f'Element {element} has attribute "{attribute_name}" with value {attribute_value}'
        # return False
    # return True
...
def element_has_attribute(element, attribute_name, attribute_value):
    return (
        element.has_attr(attribute_name)
        and element.get(attribute_name) == attribute_value
    )
